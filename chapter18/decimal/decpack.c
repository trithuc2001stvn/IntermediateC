...
DecPack * DecPack_create(int size)
{
	...
	dp -> size = size;
	dp -> used = 0;
	// allocate memory for data, should be only size/2 because
	// each byte can be stored 2 digits
	// if size is odd, increment sz by one.
	if ((size % 2) == 1)
		size ++;
...
void DecPack_insert(DecPack * dp, int val)
{
	...
	{
		// reset the lower four bits, may be let from delete
...
{
	// the object is empty, do nothing
	if (dp == NULL)
		return -1;
	// return -1 if the DecPack Object stores no data
	if ((dp -> used) == 0)
		return -1;
	// If used is even, the returned value is upper (left) 4 
	// bits. Make sure the returned value is between 0 and 9. If used is
	// add, the returned calue is the lower (rigth) 4 bits. Make sure
	// the returned value is between 0 and 9.
	int val;
	// decrement the used arrtibute in the DecPack object;
	(dp -> used) --;
	int used = dp -> used;
	if ((used % 2) == 0)
...
void DecPack_print(DecPack * dp)
{
	...
	int iter;
	int used = dp -> used;
	// go through every calue stored in the data attribute
	for (iter = 0; iter < used; iter ++)
	{
		if ((iter % 2) == 0)
