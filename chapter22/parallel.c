...
//	printf("minval = %d, maxval = %d\n", minval, maxval);
	...
	{
		numsol += subsetEqual(thd -> setA, thd -> sizeA, thd -> kval, ind);
	}
	thd -> numSol = numsol;
	return NULL;
}
...
{
	// This function does not allocate memry (malloc)
	...
	for (int = 0; ind < NUMBER_THREAD; ind ++)
	{
		int rtvl; 
		...
		total += thd[ind].numSol; // add the number of solutions
	}
	return total;
}
